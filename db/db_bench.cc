...
*/
  ...
  {
    //leveldb::open_timedif_file.open()
    //extern FILE *fp;
/*
    ...
    if (seq)
    {
      snprintf(filename, sizeof(filename), "%s", "/home/hkc/lantency_skewedZip.txt");
    }
    else
    {
      snprintf(filename, sizeof(filename), "%s", "/home/hkc/lantency_ScrambledZip.txt");
    }
...
*/
  ...
  void DoWrite_Y_U(ThreadState *thread, bool seq)
  {
    ...
    struct timespec tpend;
    long timedif;
    std::ofstream openfile("/home/hkc/lantency_Uniform.txt", std::ios::app);
    //openfile << "------------------------------------------------------------------------------------------------\n";
     */
    if (num_ != FLAGS_num)
    { // Number of key/values to place in database
  ...
  void DoWrite(ThreadState *thread, bool seq)
  {
    ...
    struct timespec tpend;
    long timedif;
    std::ofstream openfile("/home/hkc/lantency_standard.txt", std::ios::app);
*/
    if (num_ != FLAGS_num)
    { // Number of key/values to place in database
      char msg[100];
  ...
  void ReadWhileWritingZipK1(ThreadState *thread)
  {
  ...
  void LoadThenRead(ThreadState *thread){
    int RandomNum;
    generator::SkewedLatestGenerator slg(20000000); //can be modified by need
    //generator::ScrambledZipfianGenerator szg(20000000);
    generator::UniformIntegerGenerator uig(1,49999999);
    RandomGenerator gen;
    ReadOptions options;
    std::string value;
    //int time1=0;
    //while(time1 < (FLAGS_num *0.5) ){
    for (int time1 = 0; time1 < 30000000; time1++){
      const int k = uig.nextInt();
      //const int k = time1;
      char key[100];
      snprintf(key, sizeof(key), "%016d", k);
      Status s = db_->Put(write_options_, key, gen.Generate(value_size_));
    ...
    fprintf(debug,"Read: (%d found in %d)\n",found,read);
    char msg[100];
    snprintf(msg, sizeof(msg), "(%d of %d found)", found, read);
    thread->stats.AddMessage(msg);  
  }
  */
...
int checkclose(){
  for(int i=1;i<5;i++){
    if(STBs[i-1]->getEle() > STBs[i-1]->get_n()*0.08 
    && STBs[i]->getEle()/(double)STBs[i-1]->getEle()>std::pow(0.8,i)){
...
int IsFilterFull(){
  ...
  if(STBs[0]->getEle() > STBs[0]->get_n()){
      //update++;
      fprintf(fp1,"1 L0 Size:%d,L1 Size:%d,L2 Size:%d,L3 Size:%d,L4 Size:%d\n",
      STBs[0]->getEle(),STBs[1]->getEle(),STBs[2]->getEle(),STBs[3]->getEle(),STBs[4]->getEle());
      fprintf(fp1,"1 Current Req:%d,Element:[%d,%d,%d,%d,%d],STBs Size : [%f,%f,%f,%f,%f]\n",
          update,
            STBs[0]->getEle(),STBs[1]->getEle(),STBs[2]->getEle(),STBs[3]->getEle(),STBs[4]->getEle(),
            (float)STBs[0]->get_n()/1000000,(float)STBs[1]->get_n()/1000000,
            (float)STBs[2]->get_n()/1000000,(float)STBs[3]->get_n()/1000000,(float)STBs[4]->get_n()/1000000);
      //fprintf(fp1,"Req:%d,Current STBs Size : [%f,%f,%f,%f,%f]\n",
        //    update,
          //  (float)STBs[0]->get_n()/1000000,(float)STBs[1]->get_n()/1000000,
            //(float)STBs[2]->get_n()/1000000,(float)STBs[3]->get_n()/1000000,(float)STBs[4]->get_n()/1000000);
    return 2;
...
void LoadThenRead(ThreadState *thread){
    ...
    for (int time1 = 0; time1 < 30000000; time1++){
      //const int k = uig.nextInt() ;
      const int k = time1 ;
      // const int k = thread->rand.Next() % 10000000 ;
      //const int k = thread->rand.Next() % 5000000 ;
      char key[100];
      snprintf(key, sizeof(key), "%016d", k);
      Status s = db_->Put(write_options_, key, gen.Generate(value_size_));
      if (!s.ok())
    ...
    for(std::unordered_map<int,int>::iterator it=KeyMap.begin();it!=KeyMap.end();it++){
      if((*it).second>6){
        nine++;
      }else if((*it).second>4){
        six++;
    ...
    fprintf(KeyLog,"Keys Update 0~3:%d",zero);
    fprintf(KeyLog,"Keys Update 3~6:%d",tri);
    fprintf(KeyLog,"Keys Update 6~9:%d",six);      const int k = szg.nextInt();
    fprintf(KeyLog,"Keys Update 9~:%d",nine);

    
    // Op2 : Load Uniform zipfian
    

    generator::SkewedLatestGenerator slg_(2000);
    generator::ScrambledZipfianGenerator szg_(0,2000);
    FILE* zipsk=fopen("/home/hkc/zipsk.txt","a");
     FILE* zipscr=fopen("/home/hkc/zipscr.txt","a");
    for (int time1 = 0; time1 < 2000; time1++){
        char key_sk[100];
        char key_cr[100];
        //const int k = thread->rand.Next() % FLAGS_num;
        const int k_cr = szg_.nextInt();
        const int k_sk = slg_.nextInt();
        // const int k = slg.nextInt();//
        //const int k=time1;
        snprintf(key_sk, sizeof(key_sk), "%016d", k_sk);
        snprintf(key_cr, sizeof(key_cr), "%016d", k_cr);
        std::string sk=key_sk;
        std::string scr=key_cr;
        fprintf(zipsk,"%s\n",sk.c_str());
        fprintf(zipscr,"%s\n",scr.c_str());
    }
    fclose(zipsk);
...
*/
    ...
    for (int time1 = 0; time1 <0; time1++){
     ...
     //fprintf(debug,"Warm latcy:%f msc/op \n",(float)latcy_warm/20000000);
     //fprintf(debug,"Warm latcy:%f msc/op \n",(float)latcy_warm/FLAGS_num);
     WarmUpFlag=0;
     if(!STBs.empty()){
    ...
    //for (int i = 0; i < 0; i++)
    {
        if(rand()%100>0&&rand()%100<100*ReadRatio){
          ...
          //const int k =  thread->rand.Next()  % FLAGS_num;
          //const int k = szg.nextInt();
          const int k = thread->rand.Next() % 50000000;
          //const int k = szgForread.nextInt();
          snprintf(key, sizeof(key), "%016d", k);
    ...
    for (int i = 0; i < FLAGS_num; i++)
    {
      if( (rand()%100) >= 0 && (rand()%100) <= 100){
        char key[100];
        //const int k = thread->rand.Next() % 20000000;
        const int k = uig.nextInt() ;
        //const int k = uig.nextInt() ;
        //const int k = szgForread.nextInt();
        snprintf(key, sizeof(key), "%016d", k);
        ...
        //const int k = thread->rand.Next() % 4000000;
        const int k = uig.nextInt() ;
        char key[100];
        snprintf(key, sizeof(key), "%016d", k);
